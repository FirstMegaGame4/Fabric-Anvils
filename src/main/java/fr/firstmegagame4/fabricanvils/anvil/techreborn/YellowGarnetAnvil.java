...
public class YellowGarnetAnvil extends InferiorMetalAnvil {
    ...
    public YellowGarnetAnvil() {
        super(FabricBlockSettings.create().mapColor(MapColor.PALE_YELLOW).sounds(BlockSoundGroup.METAL).hardness(5.0F));
    }
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.TechRebornContent.YELLOW_GARNET_ANVIL)) {
            return TechRebornBlocks.TechRebornContent.CHIPPED_YELLOW_GARNET_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.TechRebornContent.CHIPPED_YELLOW_GARNET_ANVIL)) {
            return TechRebornBlocks.TechRebornContent.DAMAGED_YELLOW_GARNET_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
}
