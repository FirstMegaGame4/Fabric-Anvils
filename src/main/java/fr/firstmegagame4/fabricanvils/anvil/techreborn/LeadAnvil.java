...
public class LeadAnvil extends MetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.TechRebornContent.LEAD_ANVIL)) {
            return TechRebornBlocks.TechRebornContent.CHIPPED_LEAD_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.TechRebornContent.CHIPPED_LEAD_ANVIL)) {
            return TechRebornBlocks.TechRebornContent.DAMAGED_LEAD_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
}
