...
public class SapphireAnvil extends MetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.SAPPHIRE_ANVIL.get())) {
            return TechRebornBlocks.CHIPPED_SAPPHIRE_ANVIL.get().getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_SAPPHIRE_ANVIL.get())) {
            return TechRebornBlocks.DAMAGED_SAPPHIRE_ANVIL.get().getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
}
