...
public class BrassAnvil extends MetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.TechRebornContent.BRASS_ANVIL)) {
            return TechRebornBlocks.TechRebornContent.CHIPPED_BRASS_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.TechRebornContent.CHIPPED_BRASS_ANVIL)) {
            return TechRebornBlocks.TechRebornContent.DAMAGED_BRASS_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
}
