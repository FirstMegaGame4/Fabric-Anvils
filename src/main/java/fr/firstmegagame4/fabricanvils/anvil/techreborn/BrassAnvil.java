...
public class BrassAnvil extends MetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.BRASS_ANVIL.get())) {
            return TechRebornBlocks.CHIPPED_BRASS_ANVIL.get().getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_BRASS_ANVIL.get())) {
            return TechRebornBlocks.DAMAGED_BRASS_ANVIL.get().getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
}
