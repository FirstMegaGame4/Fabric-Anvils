...
public class AdvancedAlloyAnvil extends MoreAdvancedMetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.ADVANCED_ALLOY_ANVIL.get())) {
            return TechRebornBlocks.CHIPPED_ADVANCED_ALLOY_ANVIL.get().getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_ADVANCED_ALLOY_ANVIL.get())) {
            return TechRebornBlocks.DAMAGED_ADVANCED_ALLOY_ANVIL.get().getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
}
