...
public class AdvancedAlloyAnvil extends MoreAdvancedMetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.TechRebornContent.ADVANCED_ALLOY_ANVIL)) {
            return TechRebornBlocks.TechRebornContent.CHIPPED_ADVANCED_ALLOW_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.TechRebornContent.CHIPPED_ADVANCED_ALLOW_ANVIL)) {
            return TechRebornBlocks.TechRebornContent.DAMAGED_ADVANCED_ALLOW_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
}
