...
public class BronzeAnvil extends MetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.BRONZE_ANVIL.get())) {
            return TechRebornBlocks.CHIPPED_BRONZE_ANVIL.get().getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_BRONZE_ANVIL.get())) {
            return TechRebornBlocks.DAMAGED_BRONZE_ANVIL.get().getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
}
