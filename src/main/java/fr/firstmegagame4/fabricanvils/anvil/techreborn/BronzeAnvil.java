...
public class BronzeAnvil extends MetalAnvil {
    ...
    public BronzeAnvil() {
        super(FabricBlockSettings.create().mapColor(MapColor.TERRACOTTA_ORANGE).hardness(5.0F).sounds(BlockSoundGroup.METAL));
    }
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.TechRebornContent.BRONZE_ANVIL)) {
            return TechRebornBlocks.TechRebornContent.CHIPPED_BRONZE_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.TechRebornContent.CHIPPED_BRONZE_ANVIL)) {
            return TechRebornBlocks.TechRebornContent.DAMAGED_BRONZE_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
}
