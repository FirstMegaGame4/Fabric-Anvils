...
import fr.firstmegagame4.fabricanvils.anvil.AdvancedMetalAnvil;
...
public class TitaniumAnvil extends AdvancedMetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.TechRebornContent.TITANIUM_ANVIL)) {
            return TechRebornBlocks.TechRebornContent.CHIPPED_TITANIUM_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.TechRebornContent.CHIPPED_TITANIUM_ANVIL)) {
            return TechRebornBlocks.TechRebornContent.DAMAGED_TITANIUM_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
}
