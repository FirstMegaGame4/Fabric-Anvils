...
public class TitaniumAnvil extends AdvancedMetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.TITANIUM_ANVIL.get())) {
            return TechRebornBlocks.CHIPPED_TITANIUM_ANVIL.get().getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_TITANIUM_ANVIL.get())) {
            return TechRebornBlocks.DAMAGED_TITANIUM_ANVIL.get().getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
}
