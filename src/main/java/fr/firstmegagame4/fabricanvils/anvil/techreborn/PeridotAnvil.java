...
public class PeridotAnvil extends MetalAnvil {
    ...
    public PeridotAnvil() {
        super(FabricBlockSettings.create().mapColor(MapColor.LIME).sounds(BlockSoundGroup.METAL).hardness(5.0F));
    }
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.TechRebornContent.PERIDOT_ANVIL)) {
            return TechRebornBlocks.TechRebornContent.CHIPPED_PERIDOT_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.TechRebornContent.CHIPPED_PERIDOT_ANVIL)) {
            return TechRebornBlocks.TechRebornContent.DAMAGED_PERIDOT_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
}
