...
public class InvarAnvil extends MetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.INVAR_ANVIL)) {
            return TechRebornBlocks.CHIPPED_INVAR_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_INVAR_ANVIL)) {
            return TechRebornBlocks.DAMAGED_INVAR_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
