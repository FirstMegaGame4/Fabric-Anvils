...
public class TungstensteelAnvil extends AdvancedMetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.TUNGSTENSTEEL_ANVIL)) {
            return TechRebornBlocks.CHIPPED_TUNGSTENSTEEL_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_TUNGSTENSTEEL_ANVIL)) {
            return TechRebornBlocks.DAMAGED_TUNGSTENSTEEL_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
