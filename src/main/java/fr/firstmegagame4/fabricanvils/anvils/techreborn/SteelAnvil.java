...
public class SteelAnvil extends MetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.STEEL_ANVIL)) {
            return TechRebornBlocks.CHIPPED_STEEL_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_STEEL_ANVIL)) {
            return TechRebornBlocks.DAMAGED_STEEL_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
