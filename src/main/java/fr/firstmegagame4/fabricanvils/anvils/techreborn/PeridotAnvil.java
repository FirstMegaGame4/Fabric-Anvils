...
public class PeridotAnvil extends MetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.PERIDOT_ANVIL)) {
            return TechRebornBlocks.CHIPPED_PERIDOT_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_PERIDOT_ANVIL)) {
            return TechRebornBlocks.DAMAGED_PERIDOT_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
