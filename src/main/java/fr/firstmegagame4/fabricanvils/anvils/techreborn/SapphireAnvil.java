...
public class SapphireAnvil extends MetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.SAPPHIRE_ANVIL)) {
            return TechRebornBlocks.CHIPPED_SAPPHIRE_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_SAPPHIRE_ANVIL)) {
            return TechRebornBlocks.DAMAGED_SAPPHIRE_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
