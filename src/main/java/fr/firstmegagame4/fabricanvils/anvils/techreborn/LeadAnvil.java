...
public class LeadAnvil extends MetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(FATechRebornBlocks.LEAD_ANVIL)) {
            return FATechRebornBlocks.CHIPPED_LEAD_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(FATechRebornBlocks.CHIPPED_LEAD_ANVIL)) {
            return FATechRebornBlocks.DAMAGED_LEAD_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
