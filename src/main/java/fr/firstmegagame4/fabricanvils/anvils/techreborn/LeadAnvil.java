...
public class LeadAnvil extends MetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.LEAD_ANVIL)) {
            return TechRebornBlocks.CHIPPED_LEAD_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_LEAD_ANVIL)) {
            return TechRebornBlocks.DAMAGED_LEAD_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
