...
public class RubyAnvil extends MetalAnvil {
    ...
    public RubyAnvil() {
        super(FabricBlockSettings.create().mapColor(MapColor.BRIGHT_RED).sounds(BlockSoundGroup.METAL).hardness(5.0F));
    }
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.RUBY_ANVIL)) {
            return TechRebornBlocks.CHIPPED_RUBY_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_RUBY_ANVIL)) {
            return TechRebornBlocks.DAMAGED_RUBY_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
