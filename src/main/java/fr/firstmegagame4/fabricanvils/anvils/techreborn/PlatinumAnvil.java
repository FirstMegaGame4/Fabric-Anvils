...
public class PlatinumAnvil extends MetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.PLATINUM_ANVIL)) {
            return TechRebornBlocks.CHIPPED_PLATINUM_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_PLATINUM_ANVIL)) {
            return TechRebornBlocks.DAMAGED_PLATINUM_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
