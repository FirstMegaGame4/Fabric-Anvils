...
public class ZincAnvil extends MetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.ZINC_ANVIL)) {
            return TechRebornBlocks.CHIPPED_ZINC_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_ZINC_ANVIL)) {
            return TechRebornBlocks.DAMAGED_ZINC_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
