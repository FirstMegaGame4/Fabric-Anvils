...
public class RedGarnetAnvil extends InferiorMetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(FATechRebornBlocks.RED_GARNET_ANVIL)) {
            return FATechRebornBlocks.CHIPPED_RED_GARNET_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(FATechRebornBlocks.CHIPPED_RED_GARNET_ANVIL)) {
            return FATechRebornBlocks.DAMAGED_RED_GARNET_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
