...
public class RefinedIronAnvil extends MetalAnvil {
    ...
    public BlockState getStateOnLanding(BlockState fallingState) {
        if (fallingState.isOf(TechRebornBlocks.REFINED_IRON_ANVIL)) {
            return TechRebornBlocks.CHIPPED_REFINED_IRON_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else if (fallingState.isOf(TechRebornBlocks.CHIPPED_REFINED_IRON_ANVIL)) {
            return TechRebornBlocks.DAMAGED_REFINED_IRON_ANVIL.getDefaultState().with(FACING, fallingState.get(FACING));
        }
        else return null;
    }
